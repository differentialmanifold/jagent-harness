...
try {
  for (const option of process.argv.slice(2)) {
    if (!['--client-only', '--skip-build'].includes(option)) throw new Error(`Unknown option: ${option}. Supported options: --client-only, --skip-build.`)
  }
  const [major, minor] = process.versions.node.split('.').map(Number)
  if (!((major === 20 && minor >= 19) || (major === 22 && minor >= 12) || major > 22)) throw new Error('Node.js ^20.19 or >=22.12 is required')
  if (!clientOnly && (!env.JAGENT_MODEL || !env.JAGENT_OPENAI_BASE_URL)) throw new Error('Set JAGENT_MODEL and JAGENT_OPENAI_BASE_URL in .env.local; see .env.example')
  const ports = [...(!clientOnly ? [serverPort] : []), clientPort, 5175]
  ...
  if (!process.argv.includes('--skip-build')) await command('mvn', ['-q', '-DskipTests', 'package'])
  if (!existsSync(resolve(root, 'console/node_modules'))) await command('npm', ['ci'], resolve(root, 'console'))
  if (!clientOnly) {
    console.log(`Starting Java Agent server at ${serverUrl}`)
  ...
  const clientEnv = { ...env, CODING_CLIENT_PORT: String(clientPort), JAGENT_SERVER_URL: serverUrl }
  // The business client needs only its server token; model credentials belong to the server.
  ...
  console.log(`Starting Coding client at http://127.0.0.1:${clientPort}`)
  service('Coding client', 'java', ['-jar', `examples/coding-java/target/jagent-coding-java-${version}.jar`], clientEnv)
  await ready(`http://127.0.0.1:${clientPort}`)
  const ui = start(win ? 'npm.cmd' : 'npm', ['run', 'dev'], resolve(root, 'console'), { ...clientEnv, JAGENT_API_TARGET: `http://127.0.0.1:${clientPort}` })
  ui.once('error', error => { console.error(error.message); shutdown(1) })
  ui.once('exit', code => { if (!stopping) shutdown(code || 1) })
  console.log('\nJAgentHarness: open http://127.0.0.1:5175 — Ctrl+C stops these processes.\n')
} catch (error) {
