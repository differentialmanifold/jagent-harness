...
export function useAgentHarness() {
  const consoleTarget = import.meta.env.VITE_JAGENT_CONSOLE_TARGET === 'coding' ? 'coding' : 'server'
  ...
  async function loadProvider() {
    provider.value = await request('/api/v1/providers')
  }

  async function loadAgentContext(sessionId = null) {
    const body = sessionId ? { sessionId } : {}
    const context = await request('/api/v1/agent/context', {
      method: 'POST',
  ...
  async function removeSession(session) {
    ...
    try {
      await request('/api/v1/sessions/delete', {
        method: 'POST',
  ...
  async function submitRenameDialog() {
    ...
    try {
      ...
      const updated = await request('/api/v1/sessions/rename', {
        method: 'POST',
  ...
  async function selectSession(id) {
    ...
    const details = await request('/api/v1/sessions/detail', {
      method: 'POST',
    ...
    try {
      await streamRequest(
        '/api/v1/chat',
        {
          ...
          messages: [{ role: 'user', content, images: toImageRequest(images) }]
        },
        state,
  ...
  async function submitRuntimeInput(content, images, state) {
    ...
    try {
      const receipt = await request(`/api/v1/runs/${encodeURIComponent(runId)}/inputs`, {
        method: 'POST',
  ...
  async function stopMessage() {
    ...
    try {
      await request(`/api/v1/runs/${encodeURIComponent(runId)}/stop`, {
        method: 'POST'
      })
    } catch (error) {
      clearStopFallback(state)
  ...
  async function streamRequest(url, body, state, onAccepted = () => {}) {
    ...
    const response = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Accept: 'text/event-stream', 'X-Tool-Approval-Mode': approvalMode.value },
      body: JSON.stringify(body),
    ...
    if (!state.responseReceived) throw new Error("Stream ended before the committed response; inspect session state before starting another run")
  }
  ...
  async function consumeSseBlock(block, state) {
    ...
    if (eventName === 'error') throw new Error(event.message || 'Agent request failed')
    if (eventName === 'response') {
      state.responseReceived = true
      if (event.status !== 'COMPLETED' && event.status !== 'CANCELLED') appendLocalErrorMessage(state, `Run ${event.status}: ${event.answer || 'Inspect session state before retrying'}`)
      return
    }
  ...
  function handleAgentEvent(eventName, event, state, options = {}) {
    ...
    } else if (type === 'agent_requires_action') {
      // The Coding gateway keeps this run open while its SDK executes local tools.
      // Tool cards already show the useful progress; this is a protocol boundary.
  ...
  function startRun(event, payload, state) {
    ...
    if (state.activeRunId && startedAt !== null && !state.runStartTimes.has(state.activeRunId)) {
  ...
  async function resolveToolApproval({ approvalId, runId, approved }) {
    ...
    try {
      await request('/api/v1/approvals/resolve', {
        method: 'POST',
        body: JSON.stringify({
