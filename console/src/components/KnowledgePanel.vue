...
async function loadFiles() {
  ...
  error.value = ''
  try {
    files.value = await request(scopedVfsUrl('/api/v1/vfs/files', { prefix: listPrefix() }))
    await nextTick()
    if (isSkillsMode.value) {
...
async function loadContent(path) {
  const file = await request(scopedVfsUrl('/api/v1/vfs/files/content', { path }))
...
async function saveChanges() {
  ...
  try {
    ...
    const saved = await request(scopedVfsUrl('/api/v1/vfs/files'), {
      method: 'PUT',
      body: JSON.stringify({
        path,
    ...
    if (editMode.value === 'edit'
      ...
      await request(scopedVfsUrl('/api/v1/vfs/files', { path: editOriginalPath.value }), {
        method: 'DELETE'
      })
    }
    cancelEdit()
...
async function deleteCurrent() {
  ...
  try {
    ...
    for (const path of paths) {
      await request(scopedVfsUrl('/api/v1/vfs/files', { path }), {
        method: 'DELETE'
      })
    }
...
async function importSkills(uploadFile) {
  ...
  try {
    ...
    const result = await fetchJson(scopedVfsUrl('/api/v1/vfs/skills/import'), {
      method: 'POST',
      body
    })
    await loadFiles()
...
async function exportSkills() {
  ...
  notice.value = ''
  try {
    const response = await fetch(scopedVfsUrl('/api/v1/vfs/skills/export'))
    if (!response.ok) {
