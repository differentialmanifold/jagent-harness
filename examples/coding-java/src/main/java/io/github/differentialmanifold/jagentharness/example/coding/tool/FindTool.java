...
public class FindTool implements CodingTool {
    ...
    public ToolExecutionResult execute(CodingContext context, JsonNode arguments) throws Exception {
        ...
        if (!Files.isDirectory(root)) {
            throw new IllegalArgumentException(
                    "Directory not found: " + pathResolver.relative(context, root));
        }
        int limit = arguments.path("limit").asInt(DEFAULT_LIMIT);
        validateLimit(limit);

        Optional<FindResult> ripgrepResult =
                ripgrepSearchEngine.findFiles(root, pattern, limit, context.getStopSignal());
    ...
    private FindResult findWithJava(
            CodingContext context, Path root, SearchFileMatcher matcher, int limit)
            throws Exception {
    ...
    private ToolExecutionResult result(
        ...
        Collections.sort(
                sortedFiles, Comparator.comparing(path -> pathResolver.relative(context, path)));
    ...
    private String includePattern(JsonNode arguments, String name) {
        String pattern =
                pathResolver.normalizePathSeparators(ToolArguments.requiredText(arguments, name));
        if (pattern.startsWith("!")) {
            throw new IllegalArgumentException(
                    name + " must be an include glob and cannot start with !");
        }
        return pattern;
    }
