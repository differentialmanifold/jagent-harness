...
public class CodingToolConfiguration {
    ...
    @Bean
    public String codingInstructions() {
        return "### Coding Tool Usage\n"
    ...
    public RipgrepBinaryResolver ripgrepBinaryResolver(
            @Value("${coding.tools.search.rg-path:}") String configuredPath) {
    ...
    @Bean
    public RipgrepSearchEngine ripgrepSearchEngine(
            RipgrepBinaryResolver binaryResolver, RipgrepProcessRunner processRunner) {
        ...
        if (executable.isPresent()) {
            ...
            LOGGER.info(
                    "Search backend selected: Java (ripgrep was not found in the current process PATH)");
        }
