...
public class GrepTool implements CodingTool {
    ...
    public JsonNode getParametersSchema() {
        ...
        ObjectNode ignoreCase =
                ToolSchemas.booleanProperty(
                        objectMapper, "Case-insensitive search. Default false.");
    ...
    public ToolExecutionResult execute(CodingContext context, JsonNode arguments) throws Exception {
        ...
        if (!Files.exists(target)) {
            throw new IllegalArgumentException(
                    "Path not found: " + pathResolver.relative(context, target));
        }
        if (!Files.isDirectory(target) && !Files.isRegularFile(target)) {
            throw new IllegalArgumentException(
                    "Path is not a file or directory: " + pathResolver.relative(context, target));
        }
        ...
        Optional<GrepResult> ripgrepResult =
                ripgrepSearchEngine.grep(
                        target, pattern, glob, ignoreCase, literal, limit, context.getStopSignal());
        if (ripgrepResult.isPresent()) {
            return result(context, target, pattern, glob, limit, ripgrepResult.get(), "ripgrep");
        }

        GrepResult javaResult =
                searchWithJava(context, target, fileMatcher, compiledPattern, limit);
        return result(context, target, pattern, glob, limit, javaResult, "java");
    }

    private GrepResult searchWithJava(
        ...
        if (Files.isDirectory(target)) {
            try {
                        ...
                        new SimpleFileVisitor<Path>() {
                            ...
                            public FileVisitResult preVisitDirectory(
                                ...
                                if (!target.equals(directory)
                                        && SearchFileMatcher.isExcludedDirectory(directory)) {
                            ...
                            public FileVisitResult visitFileFailed(
                                    Path file, java.io.IOException exception) {
                                return FileVisitResult.CONTINUE;
                            }
                        });
                ...
                throw new IllegalStateException(
                        "Failed to search path: " + pathResolver.relative(context, target), e);
            }
        } else {
            searchFile(
                    context,
    ...
    private ToolExecutionResult result(
        ...
        Collections.sort(
                ...
                Comparator.comparing(
                                (GrepMatch match) ->
                                        pathResolver.relative(context, match.getPath()))
    ...
    private Pattern compilePattern(String pattern, boolean ignoreCase, boolean literal) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Invalid regular expression: " + e.getDescription(), e);
        }
    }
    ...
    private IllegalArgumentException unsupportedRegexFeature(String feature) {
        return new IllegalArgumentException(
                "pattern uses a regular expression feature outside the portable subset: "
                        + feature);
    }
    ...
    private String includeGlob(String value) {
        ...
        if (glob.startsWith("!")) {
            throw new IllegalArgumentException(
                    "glob must be an include glob and cannot start with !");
        }
        return glob;
    }
