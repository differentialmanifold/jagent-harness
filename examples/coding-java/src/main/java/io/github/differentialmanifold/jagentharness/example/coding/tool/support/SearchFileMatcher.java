...
public final class SearchFileMatcher {
    ...
    private static final List<String> EXCLUDED_DIRECTORY_NAMES =
    ...
    /**
     * Returns a basename-only glob that ripgrep can safely use as a temporary file type. Path-aware
     * globs are deliberately left for post-filtering.
     */
    public static Optional<String> ripgrepFileTypeGlob(String glob) {
        ...
        return candidate.indexOf('/') < 0 ? Optional.of(candidate) : Optional.<String>empty();
    }
    ...
    private static String translate(String glob, int start, int end) {
        ...
        while (index < end) {
            ...
            if (current == '*') {
                ...
                boolean completeDoubleStarSegment =
                        starEnd - index >= 2
    ...
    private static void appendCharacterClass(StringBuilder regex, String content) {
        ...
        if (content.indexOf('/') >= 0) {
            throw new IllegalArgumentException(
                    "Invalid glob pattern: character class cannot contain /");
        }
        regex.append('[');
        int index = 0;
        if (content.charAt(0) == '!') {
    ...
    private static void appendAlternatives(StringBuilder regex, String glob, int start, int end) {
        ...
        if (separators.isEmpty()) {
            throw new IllegalArgumentException(
                    "Invalid glob pattern: alternative must contain a comma");
        }
