...
public class ToolContext extends AgentContext {
    ...
    public ToolContext withLocalContext(Object value) {
        this.localContext = value;
        return this;
    }

    public <T> T requireLocalContext(Class<T> type) {
        if (!type.isInstance(localContext))
            throw new IllegalStateException("Missing local tool context: " + type.getName());
        return type.cast(localContext);
    }
    ...
    public ToolContext forToolCall(String toolCallId, String toolName) {
        ToolContext context =
                new ToolContext(
                        ...
                        toolName,
                        getProjectId());
        context.setClientCapabilities(getClientCapabilities());
        context.withLocalContext(localContext);
        return context;
    }
    ...
    public ToolApprovalDecision requestApproval(ToolApprovalRequest request) throws Exception {
        ...
        if (approvalHandler == null) {
            throw new IllegalStateException(
                    "Tool approval handler is required when approval mode is ask_for_approval");
        }
        ToolApprovalRequest effectiveRequest =
                request == null
