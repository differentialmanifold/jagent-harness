...
public class McpConfigValidator {
    ...
    private McpServerConfig validate(
            String name, McpServerConfig input, boolean resolveEnvironment) {
        if (!NAME.matcher(name == null ? "" : name).matches()) {
            throw new IllegalArgumentException(
                    "MCP server name must match [A-Za-z0-9_-]{1,64}: " + name);
        }
        if (input == null) {
        ...
        if (!McpServerConfig.STREAMABLE_HTTP.equals(config.getTransport())) {
            throw new IllegalArgumentException(
                    "Unsupported MCP transport for " + name + ": " + config.getTransport());
        }
        ...
        if (config.getConnectTimeoutSeconds() < 1
                || config.getConnectTimeoutSeconds() > 3600
                || config.getRequestTimeoutSeconds() < 1
                || config.getRequestTimeoutSeconds() > 3600) {
            throw new IllegalArgumentException(
                    "MCP timeouts must be between 1 and 3600 seconds: " + name);
        }
        if (config.getEnabledTools() != null) {
            ...
            for (String tool : config.getEnabledTools()) {
                ...
                if (toolName.isEmpty()) {
                    throw new IllegalArgumentException(
                            "MCP enabled tool name must not be empty: " + config.getName());
                }
                names.add(toolName);
            }
    ...
    private String resolveEnvironment(String value) {
        ...
        while (matcher.find()) {
            ...
            if (resolved == null) {
                throw new IllegalArgumentException(
                        "Required MCP environment variable is not set: " + matcher.group(1));
            }
